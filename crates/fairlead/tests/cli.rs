...
fn graph_why_prints_the_chain_and_importers_lists_the_edge() {
    ...
    std::fs::write(dir.join("src/b.ts"), "import { c } from './c';\nexport const b = 1;\n").unwrap();
    ...
    assert!(why.status.success(), "{}", String::from_utf8_lossy(&why.stderr));
    assert_eq!(text.lines().map(str::trim).collect::<Vec<_>>(), ["src/a.ts  (import)", "src/b.ts  (import)", "src/c.ts"]);
    let importers = fairlead_in(&dir.join("src"), &["graph", "importers", "c.ts"]);
    assert_eq!(String::from_utf8_lossy(&importers.stdout).trim(), "src/b.ts  (import)");
